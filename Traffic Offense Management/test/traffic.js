...
contract("traffic", function(accounts) {
  var Instance1;

  it("initializes the contract with the correct values", function() {
    return traffic.deployed().then(function(instance) {
        Instance1 = instance;
      return Instance1.name();
    }).then(function(name) {
      assert.equal(name, "Police Badge", "has the correct name");
      return Instance1.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, "PBN", "has the correct symbol");
      return Instance1.standard();
    }).then(function(standard) {
      assert.equal(standard, "Police Badge v1.0", "has the correct standard");
    });
  })
